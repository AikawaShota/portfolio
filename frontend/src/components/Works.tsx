...
const WorkDetailsModal = lazy(() => import("./WorkDetailsModal"));
...
const Works = forwardRef<HTMLDivElement>(function Works(_props, ref) {
    ...
    return (
        <section ref={ref} className="w-full min-h-screen bg-gray-200">
            ...
            {selectedWork ? (
                <Suspense fallback={<WorkDetailsModalFallback onClose={() => setSelectedWork(null)} />}>
...
function WorkDetailsModalFallback({ onClose }: { onClose: () => void }) {
    return (
        <dialog className="modal modal-open">
            <div className="modal-box h-[calc(100vh-2rem)] w-[calc(100vw-2rem)] max-w-6xl rounded-box bg-gray-50 px-6 py-10 md:px-10">
                <div className="mx-auto flex h-full w-full max-w-6xl items-center justify-center">
                    <span className="loading loading-spinner loading-lg" aria-label="Loading work details" />
                </div>
                <button
                    type="button"
